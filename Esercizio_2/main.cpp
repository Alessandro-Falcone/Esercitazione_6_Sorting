...
int main(int argc, char **argv){
    ...
    if(argc != 2){
        ...
        cout << "Oltre all'eseguibile che e' sempre il primo argomento passato da linea di comando deve essere passato un'altro argomento da linea di comando" << endl;
        cout << "Primo argomento linea di comando: " << argv[0] << endl;
        cout << "Secondo argomento linea di comando: <lunghezza vettore>" << endl;
        return 1;
    }else{
    ...
    for(unsigned int n = 0; n < numeroIterazioni; n++){
        ...
        auto durataBubble = duration_cast<microseconds>(fineBubble - inizioBubble); // calcolo la differenza tra il tempo di fine dell'algoritmo e il tempo di inizio dell'algoritmo usando come unità di misura i microsecondi
        bubbleDur[n] = durataBubble.count(); // mi salvo nel vettore mergeDur[n] quanto tempo ci ha messo l'algoritmo Bubble Sort per ordinare il vettore
    }
    ...
    for(unsigned int n = 0; n < numeroIterazioni; n++){
        ...
        mergeDur[n] = durataMerge.count(); // mi salvo nel vettore mergeDur[n] quanto tempo ci ha messo l'algoritmo Merge Sort per ordinare il vettore
    }
